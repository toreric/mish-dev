...
window.xdialog = function() {
    ...
    var spinOverlayElement = createSpin();
    var spinCount = 0;
    ...
    function createDialog(options, overlayElement) {
        ...
        var effect = getEffect(options.effect);
        ...
        // create innerHTML
        var innerHTML = '<div class="xd-content">';
    ...
    function createButtons(options) {
        ...
        if (utils.isArray(options.buttons)) {
            options.buttons.forEach(function(name, i) {
                var buttonInfo = predefinedButtonInfo(name);
            ...
            Object.keys(options.buttons).forEach(function(name) {
                var buttonInfo = predefinedButtonInfo(name);
                var value = options.buttons[name];
...
console.log('xdialog', buttonInfos)
        ...
        Object.keys(buttonInfos).forEach(function(name) {
            if (buttonInfos[name].html) {
                ...
                var style = buttonInfos[name].style || '';
    ...
    function create(options) {
        ...
        var dialogElement = createDialog(options, overlayElement);

        if (dialogElement === null) {
            return null;
        }

        var okButton = dialogElement.querySelector('.xd-ok');
        var cancelButton = dialogElement.querySelector('.xd-cancel');
        var deleteButton = dialogElement.querySelector('.xd-delete');

        addEventListeners();

        // load all iframes before showing
        var preparedForShow = false;
        ...
        function handleIFrame() {
            var iframes = dialogElement.querySelectorAll('iframe');
        ...
        function listenEnterAndEscKey() {
            if (options.listenEnterKey) {
                dialogElement.enterKeyListener = function listener(ev) {
                    ...
                    var topMostDialogElement = document.querySelector('.xd-dialog.xd-show');
        ...
        function adjust() {
            ...
            var old = dialogElement.style.transition;
            ...
            var rect2 = dialogElement.getBoundingClientRect();
    ...
    function createSpin() {
        ...
        var spinOverlayElement = createOverlay({
            zIndex: 2147483647
        });
    ...
    function dragElement(ondrag, destElement, srcElement, onclick) {
        ...
        function isDraggableElement(element) {
            if (ondrag) {
                var res = ondrag(element, destElement, srcElement);
